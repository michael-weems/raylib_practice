...
Application_Config application_default_config(void)
{
    ...
    c.runtime.screen_width = 1920;
    c.runtime.screen_height = 1080;
    c.runtime.target_fps = 60;
    c.runtime.title = "CPU Software Cube Field";
    ...
    c.cube_size = 1.0f;
    c.cube_spacing = 5.0f;
    c.frame_arena_size = 8u*1024u*1024u;

    c.local.render_radius = 5;
    c.local.text_radius = 3;
    c.local.grid_fine_radius = 5;
    c.local.grid_min_major_stride = 20;
    c.local.clip_near = 0.01f;
    c.local.clip_far = 8000.0f;
    c.local.orbit.mouse_sensitivity = 0.0035f;
    c.local.orbit.wheel_speed = 1.4f;
    c.local.orbit.min_pitch = -1.48f;
    c.local.orbit.max_pitch = 1.48f;
    c.local.orbit.min_distance = 2.0f;
    c.local.orbit.max_distance = 80.0f;
    c.local.orbit.fovy = 60.0f;

    c.birds_eye.block_source_span = 10;
    c.birds_eye.clip_near = 1.0f;
    c.birds_eye.clip_far = 40000.0f;
    c.birds_eye.start_distance = 7200.0f;
    c.birds_eye.orbit.mouse_sensitivity = 0.0030f;
    c.birds_eye.orbit.wheel_speed = 140.0f;
    c.birds_eye.orbit.min_pitch = -1.48f;
    c.birds_eye.orbit.max_pitch = 1.48f;
    c.birds_eye.orbit.min_distance = 600.0f;
    c.birds_eye.orbit.max_distance = 30000.0f;
    c.birds_eye.orbit.fovy = 60.0f;

    c.compass.local_length = 3.4f;
    c.compass.local_radius = 0.035f;
    c.compass.local_label_size = 0.34f;
    c.compass.bird_length_scale = 0.16f;
    c.compass.bird_radius_scale = 0.0035f;
    c.compass.bird_label_pixels = 64.0f;
...
static const sdk::Orbit_Config& active_orbit_config(Application& app)
{
    if (app.view_mode == VIEW_LOCAL) {
        return app.config.local.orbit;
    }

    return app.config.birds_eye.orbit;
}
...
static void switch_to_local(Application& app, Cube_Handle target)
{
    ...
    if (target) {
        app.active_cube = target;
    }

    Cube_Coords c = cube_coords_from_handle(app.field, app.active_cube);
    Vector3 target_center = cube_center(app.field, c.x, c.y, c.z);
    sdk::orbit_camera_retarget(app.orbit, target_center);
    if (app.orbit.distance >= app.config.local.orbit.max_distance) {
        app.orbit.distance = 12.0f;
    }
...
int application_init(Application& app, const Application_Config& config, alloc::Allocator& allocator)
{
    ...
    if (sdk::runtime_init(app.runtime, config.runtime)) {
        return 1;
    }
    ...
    if (cube_data_generate(app.data, app.field, allocator)) {
        return 3;
    }
    cube_palettes_init(app.palettes);

    uint32_t ax = app.field.count_x/2u;
    uint32_t ay = app.field.count_y/2u;
    uint32_t az = app.field.count_z/2u;
...
static void handle_keyboard(Application& app)
{
    ...
    if (IsKeyPressed(KEY_G)) {
        if (app.view_mode == VIEW_LOCAL) {
            switch_to_birds_eye(app);
        } else {
            switch_to_local(app, app.active_cube);
        }
    }
    ...
    if (key_pressed2(KEY_W, KEY_UP)) {
        navigate_active_cube(app, forward);
    }
    if (key_pressed2(KEY_S, KEY_DOWN)) {
        navigate_active_cube(app, mul(forward, -1.0f));
    }
    if (key_pressed2(KEY_D, KEY_RIGHT)) {
        navigate_active_cube(app, right);
    }
    if (key_pressed2(KEY_A, KEY_LEFT)) {
        navigate_active_cube(app, mul(right, -1.0f));
    }
}
...
{
    sdk::Text3D_Style s = {};
    s.font_size_world = 0.140f;
    s.spacing_world = 0.004f;
    s.text_color = WHITE;
...
static void draw_cube_face_text(Application& app, Cube_Handle h)
{
    Vector3 mn = {};
    Vector3 mx = {};
    cube_bounds(app.field, h, mn, mx);

    Vector3 mid = mul(add(mn, mx), 0.5f);
    char id[32];
    snprintf(id, sizeof(id), "%u", h);

    const char* value = cube_value_name(cube_value_at(app.data, h));
    float e = 0.012f;
    Vector3 px = v3(mx.x+e, mid.y, mid.z);
    Vector3 nx = v3(mn.x-e, mid.y, mid.z);
    Vector3 py = v3(mid.x, mx.y+e, mid.z);
    Vector3 ny = v3(mid.x, mn.y-e, mid.z);
    Vector3 pz = v3(mid.x, mid.y, mx.z+e);
    Vector3 nz = v3(mid.x, mid.y, mn.z-e);

    if (face_text_visible(app.orbit.camera, px, v3( 1,0,0))) {
        draw_text_stack(app.font, id, value, "+x", px, v3(0,0,-1), v3(0,1,0));
    }
    if (face_text_visible(app.orbit.camera, nx, v3(-1,0,0))) {
        draw_text_stack(app.font, id, value, "-x", nx, v3(0,0, 1), v3(0,1,0));
    }
    if (face_text_visible(app.orbit.camera, py, v3(0, 1,0))) {
        draw_text_stack(app.font, id, value, "+y", py, v3(1,0,0), v3(0,0,-1));
    }
    if (face_text_visible(app.orbit.camera, ny, v3(0,-1,0))) {
        draw_text_stack(app.font, id, value, "-y", ny, v3(1,0,0), v3(0,0, 1));
    }
    if (face_text_visible(app.orbit.camera, pz, v3(0,0, 1))) {
        draw_text_stack(app.font, id, value, "+z", pz, v3(1,0,0), v3(0,1,0));
    }
    if (face_text_visible(app.orbit.camera, nz, v3(0,0,-1))) {
        draw_text_stack(app.font, id, value, "-z", nz, v3(-1,0,0), v3(0,1,0));
    }
}
...
static int grid_stride(uint32_t count, int min_major_stride)
{
    ...
    if (stride < (uint32_t)min_major_stride) {
        stride = (uint32_t)min_major_stride;
    }
    if (!stride) {
        return 1;
    }
...
static void draw_local_grid_planes(Application& app)
{
    ...
    int radius = app.config.local.grid_fine_radius;
    int sx = grid_stride(f.count_x, app.config.local.grid_min_major_stride);
    int sy = grid_stride(f.count_y, app.config.local.grid_min_major_stride);
    int sz = grid_stride(f.count_z, app.config.local.grid_min_major_stride);

    for (uint32_t y = 0; y < f.count_y; ++y) {
        if (!grid_index_visible(y, active.y, f.count_y, radius, sy)) {
            continue;
        }
    ...
    for (uint32_t z = 0; z < f.count_z; ++z) {
        if (!grid_index_visible(z, active.z, f.count_z, radius, sz)) {
            continue;
        }
    ...
    for (uint32_t x = 0; x < f.count_x; ++x) {
        if (!grid_index_visible(x, active.x, f.count_x, radius, sx)) {
            continue;
        }
...
static void draw_local_compass(Application& app)
{
    ...
    style.font_size_world = app.config.compass.local_label_size;
    style.spacing_world = 0.01f;
...
static void render_local_view(Application& app)
{
    ...
    for (int z = minz; z <= maxz; ++z) {
        for (int y = miny; y <= maxy; ++y) {
            for (int x = minx; x <= maxx; ++x) {
                ...
                Vector3 mn = v3(center.x - half, center.y - half, center.z - half);
                Vector3 mx = v3(center.x + half, center.y + half, center.z + half);
                Local_Draw_Command cmd = {mn, mx, pal.fill[v], pal.edge[v], len2(sub(center, app.orbit.camera.position))};

                if (cmd.fill.a < 255 && transparent) {
                    transparent[transparent_count++] = cmd;
                } else {
                    draw_command_single(cmd);
                }
            }
        }
    }
    ...
    if (transparent && transparent_count) {
        ...
        for (uint32_t i = 0; i < transparent_count; ++i) {
            draw_command_single(transparent[i]);
        }
        EndBlendMode();
    }
    ...
    for (uint32_t z = 0; z < cz; ++z) {
        for (uint32_t y = 0; y < cy; ++y) {
            for (uint32_t x = 0; x < cx; ++x) {
                if (x && y && z && x+1u < cx && y+1u < cy && z+1u < cz) {
                    continue;
                }

                Vector3 mn = {};
                Vector3 mx = {};
                coarse_block_bounds(app.field, x, y, z, cx, cy, cz, mn, mx);
        ...
        for (int y = miny; y <= maxy; ++y) {
            for (int x = minx; x <= maxx; ++x) {
                ...
                if (hit.hit && hit.distance < best_dist) {
                    ...
                    best = (Cube_Handle)((uint32_t)z*app.field.stride_z + (uint32_t)y*app.field.stride_y + (uint32_t)x + CUBE_HANDLE_FIRST);
                }
            }
        }
    }
...
static Cube_Handle pick_birds_eye_cube(Application& app, Ray ray)
{
    ...
    if (best) {
        return cube_handle_from_world(app.field, best_point);
    }
...
static void handle_selection(Application& app)
{
    if (!app.cursor.wants_capture && IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) {
        ...
        Cube_Handle h = 0;
        if (app.view_mode == VIEW_LOCAL) {
            h = pick_local_cube(app, ray);
        } else {
            h = pick_birds_eye_cube(app, ray);
        }
...
static void draw_overlay(Application& app)
{
    Cube_Coords c = cube_coords_from_handle(app.field, app.active_cube);
    const Cube_Palette& pal = cube_palette(app.palettes, app.palette_index);
    int overlay_height = 108;
    if (app.cursor.wants_capture) {
        overlay_height = 82;
    }
    ...
    DrawRectangle(8, 8, 840, overlay_height, Color{0,0,0,170});
    DrawText(TextFormat("FPS %d | %s | palette %u/%u %s | active %u (%u,%u,%u)", GetFPS(), view_name, app.palette_index+1u, app.palettes.count, pal.name, app.active_cube, c.x, c.y, c.z), 16, 16, 20, RAYWHITE);
    DrawText("G: view  |  RMB: capture/select  |  LMB while free: target cube  |  1/2/3: palette  |  local WASD/arrows: snap", 16, 42, 18, LIGHTGRAY);
    if (!app.cursor.wants_capture) {
        DrawText("cursor free: camera rotation paused; click a cube to retarget, or right-click to resume rotation", 16, 70, 18, YELLOW);
    }
}
...
void application_frame(Application& app)
{
    ...
    if (IsMouseButtonPressed(MOUSE_BUTTON_RIGHT)) {
        app.cursor.wants_capture = !app.cursor.wants_capture;
    }
    ...
    if (!app.cursor.is_captured || app.cursor.suppress_delta) {
        mouse_delta = Vector2{0.0f, 0.0f};
    }

    float wheel = 0.0f;
    if (app.cursor.is_captured) {
        wheel = GetMouseWheelMove();
    }

    app.cursor.suppress_delta = 0;

    const sdk::Orbit_Config& orbit_cfg = active_orbit_config(app);
    ...
    if (app.view_mode == VIEW_LOCAL) {
        rlSetClipPlanes((double)app.config.local.clip_near, (double)app.config.local.clip_far);
    } else {
        rlSetClipPlanes((double)app.config.birds_eye.clip_near, (double)app.config.birds_eye.clip_far);
    }
    ...
    if (app.view_mode == VIEW_LOCAL) {
        ...
        render_birds_eye_shell(app);
        draw_birds_eye_compass_3d(app, bird_labels);
    }
    EndMode3D();
    if (app.view_mode == VIEW_BIRDS_EYE) {
        draw_birds_eye_compass_labels_2d(app, bird_labels);
    }
    draw_overlay(app);
    EndDrawing();
